...
export class OwnershipDiscoveryPipeline {
  ...
  public async discoverAll(options?: {
    ...
    console.log('🚀 Starting comprehensive ownership discovery pipeline...\n');
    ...
    console.log(`📋 Total BIS entities to process: ${entities.length}\n`);

    // Apply limit if specified (for testing)
    if (options?.limit) {
      entities = entities.slice(0, options.limit);
      console.log(`⚡ Limited to ${options.limit} entities for this run\n`);
    }
    ...
    if (options?.continueFromLast) {
      ...
      console.log(`📌 ${entities.length} entities remaining to process\n`);
    }
    ...
    console.log(`📊 Coverage: ${this.progress.coveragePercent.toFixed(1)}% (${this.progress.discovered}/${this.progress.totalEntities})`);
    console.log(`✓ Successful: ${this.progress.discovered}`);
  ...
  private async processInBatches(entities: BISEntityFull[], parallelism: number): Promise<void> {
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      console.log(`\n📦 Processing batch ${i + 1}/${batches.length} (${batch.length} entities)...`);
  ...
  private async discoverSingle(entity: BISEntityFull): Promise<void> {
    try {
      ...
      if (ownership.sources.length > 0) {
        ...
        console.log(`   ✓ ${entity.name}: Found ${ownership.sources.length} source(s)`);
      } else {
      ...
      console.error(`   ✗ ${entity.name}: ${error instanceof Error ? error.message : 'Unknown error'}`);
